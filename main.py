...
def chat():
    ...
    # 🧠 🆕 MARK4 PASO 3: APRENDIZAJE AUTOMÁTICO
    else:
        for word in msg.split():
            if len(word) > 4:  # filtra palabras cortas
                if "learned_words" not in memory["profile"]:
                    memory["profile"]["learned_words"] = {}

                memory["profile"]["learned_words"][word] = (
                    memory["profile"]["learned_words"].get(word, 0) + 1
                )
